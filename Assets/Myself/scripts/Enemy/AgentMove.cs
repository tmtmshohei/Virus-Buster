...
public class AgentMove : MonoBehaviour
{
	...
	void Start () 
    {
        ...
        hpy = GameObject.FindWithTag("HPy").GetComponent<RectTransform>();
        hpr = GameObject.FindWithTag("HPr").GetComponent<RectTransform>();
        damage = new Vector2(damage_offset,0);
        dataarrived = false;
        gatearrived = false;
	...
	void Update () 
    {
        ...
        Gotogate();   
        //if(hpy.offsetMax.x<=-hpy.sizeDelta.x){hpy.offsetMax = new Vector2(-hpr.offsetMax.x,0);}  
        /*
        if(!dataarrived)
        {
            agent.destination=neardata.transform.position;
